...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        elif transition == 'LA':    # dependency: first_word -> second_word
            first = self.stack[-1]
            second = self.stack[-2]
            del self.stack[-2]
            self.dependencies.append((first, second))
        
        elif transition == 'RA':    # dependency: second_word -> first_word
            first = self.stack[-1]
            second = self.stack[-2]
            del self.stack[-1]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i in range(len(parses)):
            parses[i].parse_step(transitions[i])
        
        unfinished_parses = [parse_i for parse_i in unfinished_parses if parse_i.buffer != [] or len(parse_i.stack) > 1]

    dependencies = [parse_i.dependencies for parse_i in partial_parses]
