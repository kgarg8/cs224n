...
def naiveSoftmaxLossAndGradient(
    ...
    Y = np.zeros(vocab_size)                                                    # Y = [0, 0, ..., 1, 0, ..., 0], Y[o] = 1, o: true outside word
    Y[outsideWordIdx] = 1

    Y_hat       = np.exp(np.matmul(outsideVectors, centerWordVec))              # P(O=outside word, C=center word) for all outside words
    denominator = sum(Y_hat)
    Y_hat       /= denominator

    # Alternate way: use already given softmax
    # Y_hat = softmax(np.matmul(outsideVectors, centerWordVec))

    loss            = -np.log(Y_hat[outsideWordIdx])                            # (1a) loss(softmax) = - log P(O=o, C=c)
    
    gradCenterVec   = np.matmul(outsideVectors.T, (Y_hat - Y))                  # (1b) dJ/ dv_c      = U^T.(Y_hat - Y) where U: outsideVectors
    
    gradOutsideVecs = np.matmul((Y_hat - Y)[:, None], centerWordVec[None, :])   # (1d) dJ/ dU        = (Y_hat - Y) * (v_c) where *: cross (outer) product
